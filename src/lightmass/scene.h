 ...
 * Either because one of the options have been proven superior or because it produces a
 * visualization of an intermediate result. All angles are in Radians, distances are in world space
 * units.
 */
class FStaticLightingSettings {
 public:
   ...
   * The first bounce always costs the most in terms of computation time, with the second bounce
   * following. With photon mapping, bounces after the second are nearly free.
   */
  int32 NumIndirectLightingBounces;

  /**
   * Number of skylight and emissive bounces to simulate.
   * Lightmass uses a non-distributable radiosity method for skylight bounces whose cost is
   * proportional to the number of bounces.
   */
  ...
  /** Whether to build Embree data structures for packet tracing. WIP feature - no lightmass
   * algorithms emit packet tracing requests yet. */
  bool bUseEmbreePacketTracing;

  /** Whether to use kDOP trees to accelerate volumetric lightmap voxelization. Useful in scenes
   * like large forest. */
  bool bUseFastVoxelization;

  /** Whether to use static mesh instancing to reduce memory consumption in scenes like large
   * forest. Might slow down small scenes. */
  bool bUseEmbreeInstancing;

  /**
   * Direct lighting, skylight radiosity and irradiance photons are cached on mapping surfaces to
   * accelerate final gathering. This controls the downsample factor for that cache, relative to
   * the mapping's lightmap resolution.
   */
  int32 MappingSurfaceCacheDownsampleFactor;

  /**
   * Smoothness factor to apply to indirect lighting.  This is useful in some lighting conditions
   * when Lightmass cannot resolve accurate indirect lighting. 1 is default smoothness tweaked for
   * a variety of lighting situations. Higher values like 3 smooth out the indirect lighting more,
   * but at the cost of indirect shadows losing detail as well.
   */
  float IndirectLightingSmoothness;

  /**
   * Warning: Setting this higher than 1 will greatly increase build times!
   * Can be used to increase the GI solver sample counts in order to get higher quality for levels
   * that need it. It can be useful to reduce IndirectLightingSmoothness somewhat (~.75) when
   * increasing quality to get defined indirect shadows. Note that this can't affect compression
   * artifacts or other texture based artifacts.
   */
  float IndirectLightingQuality;

  /**
   * Debugging - which single light bounce to view or -1 for all.
   * This setting has been carefully implemented to only affect the final color and not affect any
   * other part of the solver (sample positions, ray directions, etc). Most of the debug
   * visualizations are affected by ViewSingleBounceNumber.
   */
  int32 ViewSingleBounceNumber;

  /**
   * Debugging - when enabled, multiple samples will be used to detect all the texels that are
   * mapped to geometry. Otherwise only the center and corner of each texel will be sampled.
   */
  bool bUseConservativeTexelRasterization;

  /** Debugging - whether to use the texel size in various calculations in an attempt to compensate
   * for point sampling a texel. */
  bool bAccountForTexelSize;

  /** Debugging - whether to use the sample with the largest weight when rasterizing texels or a
   * linear combination. */
  bool bUseMaxWeight;
...
class FStaticLightingSceneConstants {
 public:
   ...
   * Games using a different scale should use this to convert the defaults into the game-specific
   * scale.
   */
   ...
   * This is used to push triangle shaped self shadowing artifacts onto the backfaces of curved
   * objects.
   */
   ...
   * This is applied instead of VisibilityNormalOffsetDistance whenever sample radius is known as
   * it adapts to differently sized texels.
   */
  float VisibilityNormalOffsetSampleRadiusScale;

  /**
   * Fraction of the sample radius to offset the origin of the ray in the tangent XY plane, based
   * on the direction of the ray. This is only used when bAccountForTexelSize is true.
   */
  float VisibilityTangentOffsetSampleRadiusScale;

  /**
   * Smallest texel radius allowed, useful for clamping edge cases where some texels have a radius
   * of 0. This should be smaller than the smallest valid texel radius in the scene.
   */
...
enum EViewMaterialAttribute { VMA_None, VMA_Emissive, VMA_Diffuse, VMA_Transmission, VMA_Normal };
...
struct FMeshAreaLightSettings {
  /** Whether to draw debug lines showing the corners of mesh area light primitives when a texel
   * has been selected. */
  ...
  /** Cosine of the maximum angle allowed between mesh area light primitives that get merged into
   * the same simplified primitive. */
  float MeshAreaLightSimplifyNormalCosAngleThreshold;

  /** Maximum distance allowed between any mesh area light primitive corners that get merged into
   * the same simplified primitive. */
  float MeshAreaLightSimplifyCornerDistanceThreshold;

  /** Fraction of a mesh's bounds that an emissive texel can be from a simplified primitive and
   * still get merged into that primitive. */
  float MeshAreaLightSimplifyMeshBoundingRadiusFractionThreshold;

  /** Distance along the average normal from the bounds origin of the mesh area light to place a
   * light to handle influencing dynamic objects. */
...
class FAmbientOcclusionSettings {
 public:
   ...
   * When enabled, some final gather rays will be traced even if only direct lighting is being
   * calculated.
   */
  ...
  /** Debugging - whether to only show the ambient occlusion term, useful for seeing the impact of
   * AO settings in isolation. */
  bool bVisualizeAmbientOcclusion;

  /**
   * How much of the ambient occlusion term should be applied to direct lighting.
   * A value of 0 will leave the direct lighting untouched, a value of 1 will apply all of the
   * occlusion.
   */
   ...
   * Higher powers result in more contrast, which effectively shorten the occlusion gradient and
   * push it into corners. An exponent of 1 gives equivalent occlusion as the indirect shadows from
   * a constant environment color, with no other lights in the scene.
   */
  float OcclusionExponent;

  /**
   * Fraction of the samples taken that have to be occluded before an occlusion value of 1 is
   * reached for that texel. A value of 1 means all the samples in the hemisphere have to be
   * occluded, lower values darken areas that are not fully occluded.
   */
...
class FDynamicObjectSettings {
 public:
  ...
  /** Debugging - Whether to interpolate indirect lighting for surfaces from the precomputed light
   * volume. */
  ...
  /** Maximum samples placed in the surface light sample(only for Landscape currently), used to
   * limit memory usage. */
...
class FVolumetricLightmapSettings {
 public:
   ...
   * Larger values add more resolution around geometry, improving the lighting gradients but
   * costing more memory.
   */
  ...
  /** Whether to cull bricks entirely below landscape.  This can be an invalid optimization if the
   * landscape has holes and caves that pass under landscape. */
  bool bCullBricksBelowLandscape;

  /** Subdivide bricks when a static point or spot light affects some part of the brick with
   * brightness higher than this. */
  float LightBrightnessSubdivideThreshold;

  /** Maximum desired curvature in the lighting stored in Volumetric Lightmaps, used to reduce
   * Spherical Harmonic ringing via a windowing filter. */
...
class FPrecomputedVisibilitySettings {
 public:
  ...
  /** Whether to place visibility cells only along camera tracks or to also include shadow casting
   * surfaces. */
  ...
  /** Amount to increase the bounds of meshes when querying their visibility.  Larger scales reduce
   * visibility errors at the cost of less effective culling. */
  float MeshBoundsScale;

  /** Minimum number of samples on the mesh for each cell - mesh query.  Small meshes use less
   * samples. */
  int32 MinMeshSamples;

  /** Maximum number of samples on the mesh for each cell - mesh query.  Small meshes use less
   * samples. */
...
class FVolumeDistanceFieldSettings {
 public:
  /** World space size of a voxel.  Smaller values use significantly more memory and processing
   * time, but allow more detailed shadows. */
  float VoxelSize;

  /**
   * Maximum world space distance represented in the distance field, used for normalization.
   * Larger values increase build time and decrease distance field precision, but allow distance
   * field traversal to skip larger areas.
   */
...
class FStaticShadowSettings {
 public:
  /** Debugging - Whether to filter a single shadow sample per texel in texture space instead of
   * calculating area shadows. */
  bool bUseZeroAreaLightmapSpaceFilteredLights;

  /** Number of shadow rays to trace to each area light for each texel. */
  int32 NumShadowRays;

  /** Number of shadow rays to trace to each area light once a texel has been determined to lie in
   * a shadow penumbra. */
  ...
  /**
   * Settings for enabling and adjusting a filter pass on the computed shadow factor. The shadow
   * factor is a value [0,1] that approximates the percentage of an area light visible at a texel.
   * A value of 0 indicates that the texel is completely in shadow and 1 indicates that it is
   * completely lit. The tolerance value is the maximum difference in the absolute shadow factor
   * value allowed between any two adjacent texels before enabling filtering, meant to catch shadow
   * transitions sharper than can be captured in the lightmap.
   */
   ...
   * Larger distances decrease precision but increase the maximum penumbra size that can be
   * reconstructed from the distance field.
   */
   ...
   * Higher values increase the distance field reconstruction quality, at the cost of longer build
   * times.
   */
   ...
   * Larger values increase distance field reconstruction quality on small, high resolution meshes,
  ...
  /** Amount to super sample dominant shadow map generation, in each dimension.  Larger factors
   * increase build time but produce a more conservative shadow map. */
  int32 StaticShadowDepthMapSuperSampleFactor;

  /** Maximum number of dominant shadow samples to generate for one dominant light, used to limit
   * memory used. */
  int32 StaticShadowDepthMapMaxSamples;

  /** Fraction of valid lighting samples (mapped texels or vertex samples) that must be unoccluded
   * in a precomputed shadowmap for the shadowmap to be kept. */
  float MinUnoccludedFraction;
};

/**
 * Settings related to solving the light transport equation by starting from the source of
 * importance, as opposed to starting from the source of radiant power.
 */
class FImportanceTracingSettings {
 public:
  ...
  /**
   * Number of hemisphere samples to evaluate from each irradiance cache sample when not using path
   * tracing. When photon mapping is enabled, these are called final gather rays.
   */
  int32 NumHemisphereSamples;

  /** Number of recursive levels allowed for adaptive refinement.  This has a huge impact on build
   ...
   * Useful for preventing rays nearly perpendicular to the normal which may self intersect a flat
   * surface due to imprecision in the normal.
   */
  float MaxHemisphereRayAngle;

  /** Starting threshold for what relative brightness difference causes a refinement.  At each
   * depth the effective threshold will be reduced. */
  float AdaptiveBrightnessThreshold;

  /** Starting threshold for what angle around a first bounce photon causes a refinement on all
   * cells affected.  At each depth the effective threshold will be reduced. */
  float AdaptiveFirstBouncePhotonConeAngle;

  float AdaptiveSkyVarianceThreshold;

  /**
   * Whether to use radiosity iterations for solving skylight 2nd bounce and up, plus emissive 1st
   * bounce and up. These light sources are not represented by photons so they need to be handled
   * separately to have multiple bounces.
   */
  bool bUseRadiositySolverForSkylightMultibounce;

  /**
   * Whether to cache final gather hit points for the radiosity algorithm, which reduces radiosity
   * iteration time significantly but uses a lot of memory. Memory use is proportional to the
   * lightmap texels in the scene and number of final gather rays.
   */
...
class FPhotonMappingSettings {
 public:
   ...
   * Photon mapping benefits many parts of the solver so this is mainly for comparing against other
   * methods.
   */
  bool bUsePhotonMapping;

  /**
   * Debugging - whether to estimate the first bounce lighting by tracing rays from the sample
   * point, Which is called final gathering, or by using the density of nearby first bounce
   * photons. Final gathering is slow but gets vastly better results than using first bounce
   * photons.
   */
   ...
   * This is useful to visualize direct photons.  When false, explicitly sampled direct lighting
   * will be used instead, which has less leaking.
   */
  bool bUsePhotonDirectLightingInFinalGather;

  /**
   * Debugging - whether to replace direct lighting with direct lighting as seen by the final
   * gather. This will either be direct lighting from photons, or approximate explicitly sampled
   * direct lighting, depending on bUsePhotonDirectLightingInFinalGather. This is mainly useful for
   * debugging what the final gather rays see.
   */
  bool bVisualizeCachedApproximateDirectLighting;

  /** Debugging - whether to use the optimization of caching irradiance calculations in deposited
   * photons (called Irradiance photons). */
  bool bUseIrradiancePhotons;

  /**
   * Debugging - whether to cache the result of the search for the nearest irradiance photon on
   * surfaces. This results in a constant time lookup at the end of each final gather ray instead
   * of a photon map search. Only visible photons are cached, which reduces light leaking.
   */
  ...
  /**
   * Debugging - whether to draw the photons gathered for the selected texel or the photons
   * gathered by final gather rays from the selected texel. This includes importance, irradiance
   * and direct photons.
   */
  bool bVisualizePhotonGathers;

  /**
   * Debugging - whether to draw lines in the editor showing where rays were traced due to
   * importance photons from the selected texel. If this is false, lines will be drawn for the rays
   * traced with uniform hemisphere sampling instead. Note that when irradiance caching is enabled,
   * rays will only be gathered if the selected texel created an irradiance cache sample.
   */
  bool bVisualizePhotonImportanceSamples;

  /**
   * Debugging - whether to gather and draw photon map octree nodes that get traversed during the
   * irradiance calculation for the irradiance photon nearest to the select position. Which photon
   * map search the nodes come from depends on ViewSingleBounceNumber.
   */
   ...
   * If this is false, nothing outside the volume will bounce lighting and will be lit with direct
   * lighting only.
   */
  bool bEmitPhotonsOutsideImportanceVolume;

  /** Cone filter constant, which characterizes the falloff of the filter applied to photon density
   * estimations. */
  float ConeFilterConstant;

  /** Number of photons to find in each photon map when calculating irradiance for an irradiance
   * photon. */
  int32 NumIrradianceCalculationPhotons;

  /**
   * Fraction of NumHemisphereSamples to use for importance sampling instead of uniform sampling
   * the final gather. If this fraction is close to 1, no uniform samples will be done and
   * irradiance caching will be forced off as a result. If this is 0, only uniform samples will be
   * taken.
   */
  float FinalGatherImportanceSampleFraction;

  /** Cosine of the cone angle from an importance photon direction to generate ray directions for
   * importance sampled final gathering. */
  float FinalGatherImportanceSampleCosConeAngle;

  /** World space radius of the disk around an indirect photon path in which indirect photons will
   * be emitted from directional lights. */
  float IndirectPhotonEmitDiskRadius;

  /** Angle around an indirect photon path in which indirect photons will be emitted from point,
   * spot and mesh area lights. */
   ...
   * For some scenes a small start distance (relative to MaxImportancePhotonSearchDistance) will
   * speed up the search Since it can early out once enough photons are found.  For other scenes a
   * small start distance will just cause redundant photon map searches.
   */
  float MinImportancePhotonSearchDistance;

  /** Number of importance photons to find at each irradiance cache sample that will be used to
   * guide the final gather. */
  int32 NumImportanceSearchPhotons;

  /** Scales the density at which to gather photons outside of the importance volume, if one
   * exists. */
  float OutsideImportanceVolumeDensityScale;

  /**
   * Density of direct photons to emit per light, in number of photons per million surface area
   * units.
   */
  float DirectPhotonDensity;

  /** Density of direct photons which have irradiance cached at their position, in number of
   * photons per million surface area units. */
  ...
  /**
   * Target density of indirect photon paths to gather, in number of paths per million surface area
   * units. Densities too small will result in indirect lighting not making it into small pockets.
   */
  ...
  /** Density of indirect photons which have irradiance cached at their position, in number of
   * photons per million surface area units. */
  ...
  /** Maximum cosine of the angle between the search normal and the surface normal of a candidate
   * photon for that photon to be a valid search result. */
  float PhotonSearchAngleThreshold;

  /** Cosine of the angle from the search normal that defines a cone which irradiance photons must
   * be outside of to be valid for that search. */
  ...
  /** Probability that a first bounce photon will be put into the photon segment map for volumetric
   * queries. */
...
class FIrradianceCachingSettings {
 public:
  /** Debugging - whether to allow irradiance caching.  When this is disabled, indirect lighting
   * will be many times slower. */
  bool bAllowIrradianceCaching;

  /** Debugging - whether to use irradiance gradients, which effectively allows higher order
  ...
  /** Debugging - whether to draw debug elements in the editor showing which irradiance cache
   * samples were used to shade the selected texel. */
  bool bVisualizeIrradianceSamples;

  /** Scale applied to the radius of irradiance cache records.  This directly affects sample
   * placement and therefore quality. */
  float RecordRadiusScale;

  /** Maximum angle between a record and the point being shaded allowed for the record to
   * contribute. */
  float InterpolationMaxAngle;

  /**
   * Maximum angle from the plane defined by the average normal of a record and the point being
   * shaded That the vector from the point to the record can be for that record to contribute.
   */
   ...
   * This is useful because sky occlusion tends to be less noisy than GI, so less smoothing is
   * needed to hide noise.
   */
  ...
  /**
   * Task size for parallelization of irradiance cache population within a mapping.  A mapping will
   * be split into pieces of this size which allows other threads to help. Smaller settings result
   * in more redundant irradiance cache samples, but allow better parallelization. Setting this to
   * a large value like 4096 will effectively make the irradiance caching single threaded for a
   * given mapping, which is useful for debugging.
   */
  int32 CacheTaskSize;

  /**
   * Task size for parallelization of irradiance cache interpolation within a mapping.  A mapping
   * will be split into pieces of this size which allows other threads to help. Smaller settings
   * allow better parallelization. Setting this to a large value like 4096 will effectively make
   * the irradiance cache interpolation single threaded for a given mapping, which is useful for
   * debugging.
   */
...
enum EDawnLightFlags {
  ...
  // Whether a stationary light should generate a standard shadowmap (area shadows) or a distance
  // field shadow map
...
struct FLightData {
  ...
  inline float ComputeLightProfileMultiplier(const ccl::vector<uint8> &LightProfileTextureData,
                                             FVector WorldPosition,
                                             FVector LightPosition,
                                             FVector LightDirection,
                                             FVector LightTangent) const
  {
    // optimization - only evaluate this function if needed
    if (LightFlags & Lightmass::GI_LIGHT_USE_LIGHTPROFILE) {
      FVector LightBitangent = ccl::safe_normalize(ccl::cross(LightTangent, LightDirection));
      ccl::Transform lightT = ccl::make_transform( (ccl::float3)LightDirection, (ccl::float3)LightBitangent, (ccl::float3)LightTangent );
      //FMatrix LightTransform = FMatrix(
      //    LightDirection, LightBitangent, LightTangent, FVector4(0.f, 0.f, 0.f, 1.f));
      //FMatrix InvLightTransform = LightTransform.GetTransposed();

      FVector ToLight = ccl::safe_normalize(LightPosition - WorldPosition);
      //FVector LocalToLight = InvLightTransform.TransformVector(ToLight);
      FVector LocalToLight = ccl::transform_direction_transposed(&lightT, ToLight);

      // -1..1
      float DotProd = ccl::dot(ToLight, LightDirection);
      // -PI..PI (this distortion could be put into the texture but not without quality loss or
      // more memory)
      float Angle = ccl::safe_asinf(DotProd);
      // 0..1
      float NormAngle = Angle / PI + 0.5f;

      float TangentAngle = std::atan2f(
          -LocalToLight.z,
          -LocalToLight
               .y);  // -Y represents 0/360 horizontal angle and we're rotating counter-clockwise
 ...
 private:
  ...
  // @return 0..1
  inline float FilterLightProfile(const ccl::vector<uint8> &LightProfileTextureData,
                                  const float X,
                                  const float Y) const
  {
    const uint32 SizeX = ccl::sqrt(LightProfileTextureDataSize);
    const uint32 SizeY = LightProfileTextureDataSize / SizeX;

    // can be optimized

    // not 100% like GPU hardware but simple and almost the same
    float UnNormalizedX = ccl::clamp(X * SizeX, 0.0f, (float)(SizeX - 1));
    float UnNormalizedY = ccl::clamp(Y * SizeY, 0.0f, (float)(SizeY - 1));

    uint32 X0 = (uint32)UnNormalizedX;
    uint32 X1 = ccl::min(X0 + 1, SizeX - 1);

    uint32 Y0 = (uint32)UnNormalizedY;
    uint32 Y1 = ccl::min(Y0 + 1, SizeY - 1);
    ...
    float V0 = ccl::mix(V00, V10, YFraction);
    float V1 = ccl::mix(V01, V11, YFraction);

    return ccl::mix(V0, V1, XFraction);
  }
};
...
struct FDirectionalLightData {
  /** Angle that the directional light's emissive surface extends from any receiver position, in
   * radians. */
...
struct FPointLightData {
  ...
  // Point lights need an additional axis to specify the direction of IES profiles, also used by
  // spot lights for the direction of tube lights
...
struct FSkyLightData {
  /**
   * Whether to use a filtered cubemap matching the Skylight Component's CubemapResolution to
   * represent the skylight, or a 3rd order Spherical Harmonic. The filtered cubemap is much more
   * accurate than 3rd order SH, especially in mostly shadowed areas.
   */
...
struct FMaterialElementData {
  ...
  /**
   * Typically the triangle normal is used for hemisphere gathering which prevents incorrect
   * self-shadowing from artist-tweaked vertex normals. However in the case of foliage whose vertex
   * normal has been setup to match the underlying terrain, gathering in the direction of the
   * vertex normal is desired.
   */
  uint32 bUseVertexNormalForHemisphereGather : 1;
  /** Direct lighting falloff exponent for mesh area lights created from emissive areas on this
   * primitive. */
  float EmissiveLightFalloffExponent;
  /**
   * Direct lighting influence radius.
   * The default is 0, which means the influence radius should be automatically generated based on
   * the emissive light brightness. Values greater than 0 override the automatic method.
   */
...
struct FSplineMeshParams {
  ...
  uint8 ForwardAxis; // ESplineMeshAxis::Type
};
...
struct FStaticLightingVertexMappingData {
  /** Lighting will be sampled at a random number of samples/surface area proportional to this
   * factor. */
...
class FPrecomputedVisibilityOverrideVolume {
 public:
  FBox Bounds;
  ccl::vector<int32> OverrideVisibilityIds;
  ccl::vector<int32> OverrideInvisibilityIds;
};
